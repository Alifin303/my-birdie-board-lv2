-- Function to recalculate Stableford scores for all rounds
...
BEGIN
    ...
    LOOP
        ...
        FOR hole_rec IN SELECT * FROM jsonb_array_elements(round_rec.hole_scores::jsonb)
        LOOP
            hole_strokes := COALESCE((hole_rec.value->>'strokes')::INTEGER, 0);
            hole_par := COALESCE((hole_rec.value->>'par')::INTEGER, 4);
            hole_handicap := COALESCE((hole_rec.value->>'handicap')::INTEGER, (hole_rec.value->>'holeNumber')::INTEGER);
