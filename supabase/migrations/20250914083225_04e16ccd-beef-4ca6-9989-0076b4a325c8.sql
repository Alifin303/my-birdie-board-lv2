-- Update the recalculate_all_handicaps function to use proper WHS calculation with course/tee data
DROP FUNCTION IF EXISTS public.recalculate_all_handicaps();
...
BEGIN
    ...
    LOOP
        ...
        -- Get all rounds for the user with course and tee data, ordered by date DESC (most recent first)
        FOR round_rec IN 
            SELECT 
                r.gross_score, 
                r.holes_played,
        ...
        LOOP
            -- Use course/tee data if available, otherwise fall back to defaults
            ...
            -- Adjust score for 9-hole rounds (WHS: double the score)
            IF holes_played = 9 THEN
                adjusted_score := round_rec.gross_score * 2;
            ELSE
